...
Each NodeGroup maps to one Auto Scaling Group. Every VM runs containerd, the gVisor runtime and shim, CNI plugins, upstream kubelet, CoreDNS, read-only Zot, and podmin-agent, with crictl installed for node diagnostics. runc, a Kubernetes API, CSI components, and inbound management services are absent.
...
Push preserves multi-platform indexes and uploads blobs first, `oci-layout` second, then conditionally merges and writes `index.json` last. The published top-level index also contains every nested manifest descriptor because Zot resolves digest requests there before reading blobs. A mixed-architecture cluster resolves one shared image reference to the matching manifest. Zot uses no S3 root directory and the regional AWS dual-stack endpoint. It binds only to `127.0.0.1:5000`; anonymous policy and containerd capabilities permit only pull and resolve, while VM IAM permits read-only access to `apps/` and `mirror/`. All writes go directly from the CLI to object storage. Deduplication, garbage collection, search, and metadata features are disabled. Setup derives the unique architecture set from all NodeGroup instance types and publishes each matching pause image before creating VMs.
