...
Runtime dependencies are containerd, the complete gVisor archive containing runsc, its shim and sidecar payload, CNI plugins, kubelet, CoreDNS, Zot, podmin-agent, and the pause image. AWS SSM Agent is installed from AWS's regional package bucket, configured for dual-stack service endpoints, and started for Session Manager and Run Command access. Its IAM permissions are included in the instance policy without broadening Podmin's cluster-scoped Parameter Store reads. Supported architectures are amd64 and arm64 and one cluster may use both. `crictl` is optional diagnostics, not bootstrap state.
