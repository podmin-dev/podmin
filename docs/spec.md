...
With `mtls=true`, the agent issues Fluent Bit a separate client-only, 24-hour leaf from the workload CA. Its URI SAN is `spiffe://<cluster>.podmin.internal/system/otel-logs/node/<node-id>` and it has no server-auth usage. Mode `0600` certificate and private-key files live in an atomically selected generation beneath the node tmpfs at `/run/podmin/fluent-bit`; renewal within six hours of expiry restarts Fluent Bit onto the new generation. The exporter continues to validate the provider with normal operating-system trust rather than the Podmin workload CA.
