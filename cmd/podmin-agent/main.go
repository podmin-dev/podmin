...
func main() {
	...
	workloadCAPublish := flag.String("workload-ca-publish", "", "external workload CA publication S3 object URL")
	...
	defer stop()
	if err := agent.RunDaemon(ctx, agent.DaemonConfig{Provider: *provider, Bucket: *bucket, Region: *region, Cluster: *cluster, NodeGroup: *nodeGroup, WorkloadCAPublish: *workloadCAPublish, OTelLogsCA: *otelLogsCA, OTelLogsMTLS: *otelLogsMTLS, NodeAddress: address, IPv6Prefix: podPrefix, Logger: logger}); err != nil {
